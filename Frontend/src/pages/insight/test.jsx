...
import { getInsight } from '../../services/nasaClient'
...
export default function InsightTest() {
  ...
  async function handleFetch() {
    ...
    setError(null)
    try {
      setData(await getInsight(new Date().toISOString().split('T')[0]))
    } catch (e) {
      setError(e.message)
    } finally {
      setLoading(false)
  ...
  return (
    <div className="flex flex-col items-center min-h-[calc(100vh-4rem)] px-6 py-10 gap-6">
      <h1 className="text-2xl font-semibold text-white">Insight — Mars Weather</h1>
