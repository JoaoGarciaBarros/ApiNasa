...
export default function TechportTest() {
  ...
  async function handleFetch() {
    ...
    setError(null)
    try {
      setData(await getTechport(date))
    } catch (e) {
      setError(e.message)
    } finally {
      setLoading(false)
  ...
  return (
    <div className="flex flex-col items-center min-h-[calc(100vh-4rem)] px-6 py-10 gap-6">
      <h1 className="text-2xl font-semibold text-white">TechPort</h1>
      <div className="text-white/60 text-xl mb-4">Projetos atualizados desde a data escolhida.</div>
